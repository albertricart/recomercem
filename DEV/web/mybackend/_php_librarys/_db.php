...
function openDB()
{
    ...
    // - - - - - DB Data conection
    if (file_exists("../../_data/db.php")) {
        include("../../_data/db.php");
    } else if (file_exists("../_data/db.php")) {
        include("../_data/db.php");
    }
...
function getEntity($the_table, int $the_id, int $the_sort = 0, int $the_direction = 0, int $the_limit = 0)
{
    ...
    // - - - - - Tables Data
    if (file_exists("../../_data/tb_data.php")) {
        include("../../_data/tb_data.php");
    } else if (file_exists("../_data/tb_data.php")) {
        include("../_data/tb_data.php");
    }
    ...
    switch ($the_sort) {
        case 1:
            $addStringQuery = " ORDER BY nombre" . (($the_direction == 0) ? ' ASC' : ' DESC');
            break;
        case 2:
            $addStringQuery = " ORDER BY cid" . (($the_direction == 0) ? ' ASC' : ' DESC');
            break;
        case 3:
            $addStringQuery = " ORDER BY orden" . (($the_direction == 0) ? ' ASC' : ' DESC');
            break;
        default:
            $addStringQuery = "";
    }
    ...
    if ($the_limit > 0) {
        $addStringQuery .= ' LIMIT ' . $the_limit;
    }
...
function saveEntity(string $the_table, $the_dataAry)
{
    ...
    if (empty($dbTableAry[$the_table])) {
        $isOK = false;
    }

    // controla que el array de datos no este vacio
    if (empty($the_dataAry)) {
        $isOK = false;
    }

    // genera control si es master o driver
    if ($isOK && $dbTableAry[$the_table]['tableType'] == 0) {
        $isMaster = true;
    } else {
        $isMaster = false;
    }
    ...
    if ($isOK && !empty($the_dataAry[$dbTableAry[$the_table]['tableKey']])) {
        $isNew = false;
    } else {
        $isNew = true;
    }
    ...
    if ($isOK) {
        ...
        foreach ($dbTableAry[$the_table]['tableFields'] as $tmpKey => $tmpData) {
            ...
            if (!empty($the_dataAry[$tmpKey])) {
                ...
                if ($tmpData['crypt']) {
                    $the_dataAry[$tmpKey] = crypt($the_dataAry[$tmpKey], 'magomo');
                }
