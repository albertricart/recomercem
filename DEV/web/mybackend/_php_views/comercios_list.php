...
// - - - - - - - - - - get Comerc Data
$ComerciosAry = GetIdedArray( getEntity( "comerc", 0 ) );
...
if ( !empty( $ComerciosAry ) ) {

    foreach( $ComerciosAry as $theKey => $theData ) {
...
<div class="group row">
    ...
    <!-- nombre entidad -->
    <p class="h5 text-dark px-2"><?php echo $theData['id']." - ".$theData['nombre']?></p>

    <!-- opcion Modificar -->
    <form action="comercios.html" method="POST" target="_self">
...
<!-- opcion Agregar -->    
<a href="comercios.html?idAction=<?php echo SQL_INSERT;?>" target="_self" style="display: block; position: fixed; right: 10px; bottom: 10px; width: auto; height: auto; padding: 10px; background-color: var(--primary); border-radius: 25px; z-index:100;">
